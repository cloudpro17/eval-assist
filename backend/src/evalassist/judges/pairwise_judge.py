    ...
    def _run(
        ...
        for instance in instances:
            if len(instance.responses) != 2:
                raise ValueError(
                    f"The number of texts to compare must be equal to two for all the instances. Reveived {len(instance.responses)} texts."
                )
        ...
        for criterion in criteria:
            ...
            valid_options = [
                f"{prediction_field}_a",
                f"{prediction_field}_b",
                "tie",
            ]
        ...
        predictions: list[list[str]] = [i.responses for i in instances]
        ...
        judge_description_sections = [
            f"You are a an evaluator. You are an expert on comparing {prediction_field} based on a criterion."
        ...
        prompt_template = PromptTemplate(
            ...
            template=dedent(
                ...
                1. Think step-by‑step through your reasoning about which {prediction_field} best fits. Take into account that a tie is possible and should be taken into account.
                ...
                - {prediction_field}_a: {text_a}
                - {prediction_field}_b: {text_b}

                ## Tie option

                If both of the {prediction_field} texts are equally good based on the criteria, set the `"selected_option"` field as `"tie"`.
        ...
        prompts: list[str] = [
            ...
            for (
                text_a,
                text_b,
            ), context_section, criterion, format_instructions, prediction_field, judge_description_section in zip(
                predictions,
                context_sections,
                criteria,
                format_instructions_list,
